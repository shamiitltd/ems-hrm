...
const Hiringpipeline = () => {
  ...
  const sectionData = [
    {
      Job: {
        job1: "Senior product design",
        application: "total application 125",
      },
    ...
    {
      Job: {
        job1: "Juniour product design",
        application: "total application 12",
      },
  ...
  return (
    <div className="bg-white text-dark mx-5 my-5 rounded p-3">
      ...
      {sectionData.map((data, index) => (
        <div
          key={index}
          className="d-flex flex-column flex-md-row align-items-start mb-2"
        >
          {headingData.map((heading, idx) => (
            <div
              key={heading}
              className={`text-dark py-1 px-2 w-100 ${
                heading === "Job" ? "d-flex flex-column" : ""
              }`}
            >
              {heading === "Job" ? (
                <>
                ...
                <div
                  className={`card p-2 ${
                    colors[(index + idx) % colors.length]
                  }`}
                >
                  {data[heading].count}
                </div>
